...
constraints_1 <- constraints[[1]]
constraints_2 <- constraints[[2]]
constraints_3 <- constraints[[3]]
...
server <- function(input, output, session) {
  ...
  # Define parameters
  nb_of_class_1 <- ncol(HEC_dummies_BA[[1]])
  nb_of_timeslot_1 <- dim(HEC_dummies_BA[[1]] %>%
  filter(grepl("name", rownames(HEC_dummies_BA[[1]]))))[1]
  nb_of_moment_1 <- length(row.names(HEC_dummies_BA[[1]] %>%
  filter(grepl("AM|PM", rownames(HEC_dummies_BA[[1]])))))

  # Set matrix corresponding to coefficients of constraints
  f.con_1 <- HEC_dummies_BA[[1]]

  # Set coefficients of the objective function (min total credits taken)
  f.obj_1  <- HEC_dummies_BA[[1]][1,] %>% unname() %>% unlist()
  ...
  # Define parameters
  nb_of_class_2 <- ncol(HEC_dummies_BA[[2]])
  nb_of_timeslot_2 <- dim(HEC_dummies_BA[[2]] %>%
  filter(grepl("name", rownames(HEC_dummies_BA[[2]]))))[1]
  nb_of_moment_2 <- length(row.names(HEC_dummies_BA[[2]] %>%
  filter(grepl("AM|PM", rownames(HEC_dummies_BA[[2]])))))

  # Set matrix corresponding to coefficients of constraints
  f.con_2 <- HEC_dummies_BA[[2]]

  # Set coefficients of the objective function (min total credits taken)
  f.obj_2  <- HEC_dummies_BA[[2]][1,] %>% unname() %>% unlist()
  ...
  # Define parameters
  nb_of_class_3 <- ncol(HEC_dummies_BA[[3]])
  nb_of_timeslot_3 <- dim(HEC_dummies_BA[[3]] %>%
  filter(grepl("name", rownames(HEC_dummies_BA[[3]]))))[1]
  nb_of_moment_3 <- length(row.names(HEC_dummies_BA[[3]] %>%
  filter(grepl("AM|PM", rownames(HEC_dummies_BA[[3]])))))

  # Set matrix corresponding to coefficients of constraints
  f.con_3 <- HEC_dummies_BA[[3]]

  # Set coefficients of the objective function (min total credits taken)
  f.obj_3  <- HEC_dummies_BA[[3]][1,] %>% unname() %>% unlist()
